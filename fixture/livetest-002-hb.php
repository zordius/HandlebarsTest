...
'funcs' => Array(
    ...
    ob_start();echo '',$cx['funcs']['sec']('specs', $cx, $in, true, function($cx, $in) {echo '
   <div class="item-spec">
    ',$cx['funcs']['sec']('options', $cx, $in, true, function($cx, $in) {echo '
        ',$cx['funcs']['ifv']('icon', $cx, $in, function($cx, $in) {echo '
        <div class="spec">
            <div class="spec-overlay"></div>
            <input type="radio" id="spec-',$cx['funcs']['enc']('../id', $cx, $in),'-',$cx['funcs']['enc']('id', $cx, $in),'" value="',$cx['funcs']['enc']('id', $cx, $in),'" name="',$cx['funcs']['enc']('../id', $cx, $in),'" alt="',$cx['funcs']['enc']('name', $cx, $in),'" data-name="',$cx['funcs']['enc']('name', $cx, $in),'"/>
            <label class="spec-label icon" for="spec-',$cx['funcs']['enc']('../id', $cx, $in),'-',$cx['funcs']['enc']('id', $cx, $in),'" style="background-image:url(',$cx['funcs']['enc']('icon', $cx, $in),');">
            </label>
        </div>
        ';}, function($cx, $in) {echo '
        <div class="spec">
            <div class="spec-overlay"></div>
            <input type="radio" id="spec-',$cx['funcs']['enc']('../id', $cx, $in),'-',$cx['funcs']['enc']('id', $cx, $in),'" value="',$cx['funcs']['enc']('id', $cx, $in),'" name="',$cx['funcs']['enc']('../id', $cx, $in),'" data-name="',$cx['funcs']['enc']('name', $cx, $in),'"/>
            <label class="spec-label" for="spec-',$cx['funcs']['enc']('../id', $cx, $in),'-',$cx['funcs']['enc']('id', $cx, $in),'">',$cx['funcs']['enc']('name', $cx, $in),'</label>
        </div>
        ';}),'
        <script>
            document.getElementById(\'spec-',$cx['funcs']['enc']('../id', $cx, $in),'-',$cx['funcs']['enc']('id', $cx, $in),'\').className += \'hidden\';
        </script>
    ';}),'
    </div>
';}),'
